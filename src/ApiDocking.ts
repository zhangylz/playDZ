...
class ApiDocking {
    ...
    /** 获取每星期签到列表 */
    public getSign(): Promise<any> {
        let that = this;
        return new Promise((resole, erject) => {
            wx.request({
    ...
    public authLogin(is_auth: boolean): void {
        ...
        if (!is_auth) {
            console.log("用户没有授权");
            // console.log(that.userToken);
            let startSkin:string = Laya.ResourceVersion.addVersionPrefix(this.Game.gameHome.starGame.skin);
